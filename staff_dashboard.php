...
// Check if staff member has registered for a bus
$sql_check_registration = "SELECT * FROM bus_registrations WHERE user_id_staff = {$staff['id']}";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['bus_id'])) {
    ...
    $sql_insert_registration = "INSERT INTO bus_registrations (user_id_staff, bus_id, registration_date) VALUES ({$staff['id']}, $bus_id, '$registration_date')";
...
<header class="header-outer">
	<div class="header-inner responsive-wrapper">
        ...
        <h2>Welcome, <?php echo $staff['staff_id']; ?></h2>
		</div>
...
<main class="main">
	<div class="main-content responsive-wrapper">
        ...
        <p style="font-weight : bold;">Email : <?php echo $staff['email']; ?></p>
        <p style="font-weight : bold;">Username: <?php echo $staff['staff_id']; ?></p>
        <?php if ($registered) { ?>
            <br>
                ...
                <select name="bus_id" style="padding: 10px; font-size: 16px; border-radius: 5px;">
                    ...
                    if ($result_fetch_buses && mysqli_num_rows($result_fetch_buses) > 0) {
                        while ($bus = mysqli_fetch_assoc($result_fetch_buses)) {
                            // Display bus details as options in the select dropdown
